            ...
            if let Some(desired_tire_type) = runtime_store.plan_pit_fix_tire_apply(
                ...
                if let Some(engine_tire_type) = engine_tire_type_from_runtime(desired_tire_type) {
                        ...
                        .await
                    {
                        ...
                        Err(err) => {
                            tracing::warn!(
                                ...
                                error = %err,
                                "frame hub: failed to apply pit-fix tyre type"
                            );
                        }
                    }
                } else {
                    tracing::warn!(
                        ...
                        target = ?target,
                        desired_tire_type = ?desired_tire_type,
                        "frame hub: skipped pit-fix tyre apply for unspecified tyre type"
                    );
                }
            }
        }
...
fn engine_tire_type_from_runtime(value: RuntimePitTireType) -> Option<TyreType> {
    match value {
        ...
        RuntimePitTireType::Soft => Some(TyreType::Soft),
        RuntimePitTireType::Wet => Some(TyreType::Wet),
        RuntimePitTireType::Unspecified => None,
    }
}
