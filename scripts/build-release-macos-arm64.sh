...
print_usage() {
  cat <<'EOF'
Usage: build-release-macos-arm64.sh [options]
...
copy_required_files() {
  ...
  if [[ -d "$target_release_dir" ]]; then
    find "$target_release_dir" -maxdepth 1 -type f -name "*.dylib" -exec cp -f {} "$package_root/" \;
  fi

  if [[ -d "$native_runtime_dir" ]]; then
    find "$native_runtime_dir" -maxdepth 1 -type f -name "*.dylib" -exec cp -f {} "$package_root/" \;
  fi
...
NATIVE_RUNTIME_DIR="$REPO_ROOT/game-engine/boink-sys/native/macos/aarch64/release"
