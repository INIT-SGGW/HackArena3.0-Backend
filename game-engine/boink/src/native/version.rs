...
///
/// Legacy mode supports only native libraries exposing C API `>= 0.17.7`.
#[cfg(feature = "legacy-native-lib")]
pub const MIN_LEGACY_C_API_VERSION: Version = Version::new(0, 17, 7);
...
pub fn ensure_c_api_compatible() -> Result<()> {
    ...
    LEGACY_WARN_ONCE.call_once(|| {
        warn!("Legacy compatibility mode enabled; minimum supported C API is 0.17.7");
    });
